...
# (v2.0 - CLEAN Teleportation-Based Evaluation / NO HEURISTICS)
# 
# PURPOSE:
...
#   This version uses PURE MODEL PREDICTIONS without any heuristic assistance.
#
# VERIFIED DATA COMPATIBILITY:
#   - prev_image: Transformed to match training (224x224, normalized [-1,1])
#   - curr_image: Transformed to match training (224x224, normalized [-1,1])
#   - goal_image: Last frame of trajectory (robot retracted, object at goal)
#   - curr_proprio: [qpos[:7], qvel[:7], touch_sensors[2], force_vectors[6]]
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] [TELEPORT-v2] %(message)s",
    handlers=[
...
# ==============================================================================
# 1. GRIPPER HANDLER (DIRECT QPOS CONTROL - NO HEURISTICS)
# ==============================================================================

class GripperHandler:
    """
    Manages gripper state via direct qpos manipulation.
    Uses Schmitt trigger for hysteresis (prevents oscillation).
    
    Training convention: gt_grip_chunk = 1.0 (CLOSED), 0.0 (OPEN)
    Model outputs logits -> sigmoid -> probability
    """
    
    GRIPPER_OPEN_QPOS = 0.04
    GRIPPER_CLOSED_QPOS = 0.0
    
    # Schmitt trigger thresholds for hysteresis
    CLOSE_THRESHOLD = 0.55   # Close when prob > 0.55
    OPEN_THRESHOLD = 0.45    # Open when prob < 0.45
    
    def __init__(self):
        self.is_closed = False
        self.smoothed_prob = 0.0
        self.alpha = 0.3  # EMA smoothing factor (lower = more smoothing)
    ...
    def update(self, grip_logit: float) -> Tuple[float, bool]:
        ...
        # Schmitt trigger decision (hysteresis)
        if not self.is_closed and self.smoothed_prob > self.CLOSE_THRESHOLD:
            self.is_closed = True
        elif self.is_closed and self.smoothed_prob < self.OPEN_THRESHOLD:
            self.is_closed = False
...
def render_goal_image_like_training(env: PandaEnv, goal_pos_world: np.ndarray):
    ...
    Training: goal_image = all_images[-1] (last frame of successful trajectory)
    """
    ...
    try:
        # Robot in home/retracted position (matches end of successful trajectory)
...
def teleport_robot(env: PandaEnv, ik_solver: IKSolver, 
    ...
    target_pose_for_ik = target_pose_7d  # IKSolver expects [x,y,z,qx,qy,qz,qw]
    ...
    try:
        target_joints = ik_solver._get_target_joint_angles(
            ...
            solution_position_tolerance=0.05,
            max_iter=50
        )
    ...
    # Run physics substeps to let environment settle
...
# ==============================================================================
# 4. MAIN EVALUATOR (PURE MODEL PREDICTIONS - NO HEURISTICS)
# ==============================================================================

class TeleportEvaluator:
    """
    Clean Teleportation-Based Semantic Planner Evaluator.
    Uses ONLY model predictions without any heuristic assistance.
    """
    ...
    def __init__(self, cfg: DictConfig):
        ...
        # --- D. Image Transform (MUST MATCH TRAINING EXACTLY) ---
        # Training: transforms.Normalize(mean=[0.5,0.5,0.5], std=[0.5,0.5,0.5])
        # Converts [0,1] -> [-1,1]
    ...
    def _get_proprio_exact(self) -> np.ndarray:
        """
        Get proprio vector in EXACT format matching training data.
        ...
        # Touch sensors (same as PandaEnv._get_obs)
        ...
        # Force sensors (same as PandaEnv._get_obs)
    ...
    def _get_observation(self) -> Dict[str, np.ndarray]:
        ...
        # Get proprio (EXACT match to training)
        proprio = self._get_proprio_exact()
        
        # Check grasp
        is_grasped = self.env._is_physically_grasped if hasattr(self.env, '_is_physically_grasped') else False
    ...
    def run(self):
        ...
        telemetry_writer.writerow([
            ...
            "obj_x", "obj_y", "obj_z", "is_grasped", "teleport_ok"
        ])
        
        log.info(f"Starting CLEAN TELEPORT Evaluation (NO HEURISTICS)")
        log.info(f"Episodes: {self.cfg.num_episodes}, Max Steps: {self.cfg.max_steps}")
        ...
        try:
            ...
            log.info("=" * 60)
            log.info(f"TELEPORT EVALUATION REPORT (NO HEURISTICS)")
    ...
    def run_episode(self, ep_idx: int, seed: int, 
        ...
        # 3. Initialize History (prev_image = curr_image at t=0)
        ...
        for step in range(self.cfg.max_steps):
            ...
            # --- B. Prepare Model Input (EXACT match to training) ---
            curr_img_pil = Image.fromarray(obs['image_primary'])
            ...
            if step < 5 and ep_idx == 0:
                log.info(f"Step {step}: pos={target_pose_raw[:3]}, grip_logit={target_grip_logit:.2f}, phase={predicted_phase}")
            
            # --- D. Gripper Decision (PURE MODEL - NO HEURISTICS) ---
            ...
            # --- E. TELEPORT Robot (PURE MODEL PREDICTION) ---
            ...
            telemetry_writer.writerow([
                ...
                int(is_grasped), int(teleport_ok)
            ])
            ...
            cv2.putText(frame_bgr, f"Grip: {'CLOSE' if gripper_closed else 'OPEN'} ({grip_prob:.2f})", 
