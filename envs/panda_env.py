...
class DomainRandomizationConfig:
    ...
    camera_shots: List[CameraShot] = field(default_factory=lambda: [
        ...
        CameraShot(pos=(0.95, 0.0, 0.7), target=(0.6, 0.05, 0.5)),
...
class PandaEnv(gym.Env):
    ...
    def _apply_domain_randomization(self):
        """Randomizes textures, lighting, and camera. Uses curated shots with robust spherical jitter."""
        if not self.enable_domain_randomization:
            return
        ...
        if not self.dr_config.camera_shots:
            ...
            # [cite_start]1. Select a known-good shot from the curated list[cite: 3].
            ...
            # 2. Convert the base shot to spherical coordinates.
            chosen_shot = self.np_random.choice(self.dr_config.camera_shots)
            base_pos = np.array(chosen_shot.pos)
            base_target = np.array(chosen_shot.target)

            # 2. Apply a simple, direct jitter to the camera's world position.
            # This is more stable than spherical jitter for these specific shots.
            pos_jitter = self.np_random.uniform(-0.06, 0.06, size=3)
            final_cam_pos = base_pos + pos_jitter

            # 3. Jitter the target point, but constrain it to stay near the table surface.
            # This is the KEY to preventing the camera from looking away from the action.
            target_jitter = self.np_random.uniform(-0.1, 0.1, size=2)
            final_target = base_target.copy()
            final_target[:2] += target_jitter # Jitter XY
            final_target[2] = self.np_random.uniform(0.4, 0.5) # Constrain Z to table height

            # Now, use the final corrected position and target for the camera.
            cam_pos = final_cam_pos
            target = final_target


            # --- The rest of the function remains the same ---
            new_quat_xyzw = self._calculate_look_at_quat(cam_pos, target)
