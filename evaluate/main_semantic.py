...
# (v13.0 - Transport Safety Protocol & Ratchet Logic)
        ...
        try:
            for ep_idx in tqdm(range(self.cfg.num_episodes), desc="Evaluating"):
                ...
                for step in range(self.cfg.max_steps):
                    ...
                    # --- v13.0 PHYSICS-AWARE SEQUENCER (Safety Protocol) ---
                    ...
                    # If we have the object, NEVER let go.
                    if is_physically_grasped:
                        heuristic_active = 3 # Status 3: Transport
                        raw_grip_logit = 5.0 # Force CLOSE
                        # We let the policy control position (to get to goal),
                        # but we physically enforce the grip.
                    
                    else:
                        # If NOT grasped, run the acquisition logic
                        ...
                        if grasp_latch_counter > 0:
                            ...
                            if grasp_latch_counter > 25:
                                ...
                                # Gentle Lift Fix: Reduce gain to prevent jerk
                                current_action_gain = 1.0 

                            grasp_latch_counter -= 1

                    # Safety Clamp (Always active)
                    ...
                    is_grasped_now = obs['is_grasped'][0] > 0.5
                    
                    if dist_goal < 0.05 and is_lifted and is_grasped_now:
                    ...
                    # Color Coding: Green=Approach, Red=Latch, Blue=Transport
                    color = (255, 255, 255)
                    if heuristic_active == 1: color = (0, 255, 0)   # Green
                    if heuristic_active == 2: color = (0, 0, 255)   # Red
                    if heuristic_active == 3: color = (255, 0, 0)   # Blue
                    ...
                    writer.writerow([
                        ...
                        f"{dist_3d:.4f}", is_grasped_now, heuristic_active
                    ])
